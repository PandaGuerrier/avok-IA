...
export default function EmailDetail({ selectedEmail, setSelectedEmail }: EmailDetailProps) {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 bg-white dark:bg-slate-900 transition-colors">
      <div className="flex items-center gap-4 mb-6 border-b border-sky-50 dark:border-slate-800 pb-4">
        <button onClick={() => setSelectedEmail(null)} className="p-2 hover:bg-sky-50 dark:hover:bg-slate-800 text-sky-600 dark:text-slate-400 rounded-full transition-colors">
          ← Retour
        </button>
      </div>
      ...
      <div className="flex items-center justify-between mb-8 px-2">
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg ${getAvatarColor(selectedEmail.sender)}`}>
      ...
      <div className="text-sky-800 dark:text-slate-300 px-2 whitespace-pre-wrap leading-relaxed" dangerouslySetInnerHTML={{ __html: selectedEmail.body }}></div>
        ...
        <div className="mt-10 px-2 pt-6 border-t border-sky-100 dark:border-slate-800">
          ...
          <div className="flex flex-wrap gap-4">
            {selectedEmail.attachments.map((att, index) => (
              <div key={index} className="flex items-center gap-3 p-3 w-64 bg-sky-50/50 dark:bg-slate-800/50 rounded-xl border border-sky-100 dark:border-slate-700 hover:bg-sky-100 dark:hover:bg-slate-700 cursor-pointer transition-colors group">
                <div className="w-10 h-10 flex items-center justify-center bg-white dark:bg-slate-900 rounded-lg text-2xl shadow-sm border border-sky-50 dark:border-slate-800 group-hover:scale-105 transition-transform">
                  📄
                </div>
