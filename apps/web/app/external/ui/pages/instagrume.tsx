...
const Instagrume: React.FC = () => {
  const { gameUuid, insta, contacts } = usePageProps<Props>()
    ...
    <div className="flex h-[calc(100vh-4rem)] bg-white dark:bg-gray-900 text-black dark:text-white font-sans overflow-hidden">
      ...
      <main
        className={`flex-1 flex flex-col h-full transition-all ${
          activeTab === 'messages' ? 'items-stretch overflow-hidden' : 'items-center p-4 md:p-8 overflow-y-auto'
        }`}
      >
        {/* FEED */}
        {activeTab === 'feed' && (
            ...
            <div className="flex-1 flex flex-col bg-white dark:bg-gray-900">
              {selectedContact ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-center p-10 bg-gray-50/30 dark:bg-gray-800/30">
                  ...
                  <h3 className="text-xl font-medium text-gray-900 dark:text-white">Sélectionne une conversation</h3>
                  <p className="text-gray-400 dark:text-gray-500 text-sm mt-2">Clique sur des messages pour les utiliser comme alibi</p>
                </div>
              )}
            </div>
          </div>
