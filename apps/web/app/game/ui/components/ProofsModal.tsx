...
export default function ProofsModal({
  ...
  return (
    <div className="absolute inset-0 z-50 flex items-start justify-center pt-16 px-4 bg-black/60 dark:bg-black/60 backdrop-blur-md">
      <div className="w-full max-w-2xl bg-white dark:bg-[#0a0a1a] border border-gray-200 dark:border-white/15 rounded-2xl shadow-2xl flex flex-col max-h-[75vh]">
        {/* Header */}
        <div className="shrink-0 px-5 py-4 border-b border-gray-200 dark:border-white/10 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-gray-900 dark:text-white uppercase tracking-wider">Dossier à charge</h2>
            <span className="bg-cyan-100 dark:bg-cyan-500/15 text-cyan-700 dark:text-cyan-400 text-[10px] px-2 py-0.5 rounded-full border border-cyan-300 dark:border-cyan-500/20">
              ...
              return (
                <div
                  ...
                  className={`rounded-xl border transition-all ${
                    isSelected
                      ? 'bg-cyan-100 dark:bg-cyan-500/8 border-cyan-300 dark:border-cyan-500/30'
                      : 'bg-gray-50 dark:bg-white/3 border-gray-200 dark:border-white/8 hover:border-gray-300 dark:hover:border-white/15'
                  }`}
                >
                  <div className="flex items-center gap-3 px-4 py-3">
                    <button
                      ...
                      className={`w-4 h-4 rounded border shrink-0 transition-all ${
                        isSelected
                          ? 'bg-cyan-600 dark:bg-cyan-500 border-cyan-600 dark:border-cyan-500'
                          : 'bg-transparent border-gray-300 dark:border-white/20 hover:border-cyan-500/50'
                      }`}
                    >
                      {isSelected && (
                        <span className="flex w-full h-full items-center justify-center text-white text-[10px] font-bold leading-none">
                          ✓
                        </span>
                      )}
                    </button>
                    <Icon className={`w-4 h-4 shrink-0 ${isSelected ? 'text-cyan-600 dark:text-cyan-400' : 'text-gray-400 dark:text-white/40'}`} />
                    <div className="flex-1 min-w-0">
                      <p className={`text-xs font-semibold ${isSelected ? 'text-cyan-700 dark:text-cyan-300' : 'text-gray-800 dark:text-white/80'}`}>
                        {proof.data?.title || 'Preuve'}
                      </p>
                      <p className="text-[10px] text-gray-500 dark:text-white/30 uppercase tracking-wider mt-0.5">
                        {proof.data?.type || 'document'}
                      </p>
                    </div>
                    <button
                      onClick={() => setExpandedUuid(isExpanded ? null : proof.uuid)}
                      className="p-1 text-gray-400 dark:text-white/30 hover:text-gray-600 dark:hover:text-white/60 transition-colors"
                    >
                      {isExpanded ? (
                  ...
                  {isExpanded && (
                    <div className="px-4 pb-3 pt-0 border-t border-gray-200 dark:border-white/8 mt-2">
                      {proof.data?.type === 'image' && proof.imageUrl ? (
                        ...
                        <a
                          ...
                          className="text-xs text-cyan-600 dark:text-cyan-400 underline mt-2 inline-block"
                        >
                          Ouvrir le PDF
                        </a>
                      ) : (
                        <p className="text-xs text-gray-600 dark:text-white/60 leading-relaxed mt-2">{proof.content}</p>
                      )}
                    </div>
                  )}
                </div>
        ...
        {selectedProofUuids.length > 0 && (
          <div className="shrink-0 px-5 py-3 border-t border-gray-200 dark:border-white/10 bg-cyan-50 dark:bg-cyan-500/5 flex items-center justify-between">
