...
export function ProfileForm({ user }: Props) {
  ...
  const { setData, errors, put, progress } = useForm({
        ...
        <FieldGroup>
          ...
          <Field>
            ...
            <span className={'text-gray-500 text-xs'}>
              Pour changer son nom sur le site, vous devez faire une demande (pour éviter que vous
              n'utilisiez pas votre vrai prénom / nom)
            </span>
            <span className={'text-gray-500 text-xs'}>
              Envoie un mail à <span className={"text-primary"}>jules.lofficial@efrei.net</span> avec comme objet:{' '}
              <span className={'text-destructive text-xs'}>Changement nom Eaglet</span>
            </span>
          </Field>
